...
def check_state(required_field: bool) -> PackerConfig:
...
if st.session_state.form_submitted and not st.session_state.form2_submitted:
    ...
    with st.form("template_setup"):
        ...
        with col1:
            disk_size = st.number_input(
                "choose disk size in G:", min_value=1, max_value=999, placeholder="32"
            )
    ...
    if template_submit:
        ...
        with st.spinner("Packer Deployment in progress..."):
            conf = check_state(all_required_fields_filled)
            deployment(
                conf=conf,
                logger=logger,
